...
function $query(ctx, query, values, qrm, config) {
    ...
    if (!query) {
        error = new TypeError('Invalid query format.');
    }
    ...
    if (!error && !special) {
        if (npm.utils.isNull(qrm)) {
            ...
            if (qrm !== parseInt(qrm) || qrm < npm.result.one || qrm > npm.result.any) {
                ...
                if (err) {
                    ...
                    if (multiResult) {
                        for (let i = 0; i < result.length; i++) {
                            ...
                            npm.utils.addReadProp(r.rows, 'duration', result.duration, true);
                        }
                    } else {
                        ...
                        if (special) {
                            if (multiResult) {
                                if (special.isMultiResult) {
                                    ...
                                    error = new QueryResultError(qrec.multiResult, result, query, params);
                                }
                            } else {
                                if (special.isResult) {
                                    data = result;
                            ...
                            // not special
                            if (multiResult) {
                                if (qrm & npm.result.multi) {
                                    data = result.map(r => r.rows);
                                } else {
                                    error = new QueryResultError(qrec.multiResult, result, query, params);
                                }
                            } else {
                                ...
                                if (len > 1) {
                                    if (qrm & npm.result.many) {
                                        ...
                                        if (qrm & npm.result.multi) {
                                            ...
                                            error = new QueryResultError(qrec.multiRow, result, query, params);
                                        }
                                    }
                                } else {
                                    if (len === 1) {
                                        ...
                                        if (qrm & npm.result.none) {
                                            data = (qrm & npm.result.many) ? result.rows : null;
                                        } else {
                                            if (qrm & npm.result.multi) {
                                                data = [result.rows];
                                            } else {
